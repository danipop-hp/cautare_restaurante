<script>
  ...
  function handleSubmit(event) {
    ...
    if (!result.ok) {
      error = result.message ?? 'Nu am putut crea contul.';
      return;
    }
    goto('/login');
  }
...
<main class="auth-main">
  <section class="auth-card">
    <div class="auth-header">
      <p class="auth-brand">Urban<span>Plate</span></p>
      <h1>Creeaza cont</h1>
      <p class="auth-subtitle">Activeaza accesul la cautare si recomandari.</p>
    </div>

    <form class="auth-form" onsubmit={handleSubmit}>
      <label>
        Nume
        <input type="text" bind:value={name} placeholder="Nume complet" required />
      </label>
      <label>
        Email
        <input type="email" bind:value={email} placeholder="nume@email.ro" required />
      </label>
      <label>
        Parola
